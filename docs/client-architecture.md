...
`Wandur.Core.Channels` holds the recognition. A `ChannelRule` is a channel name, a .NET regular expression with the optional named groups `speaker` and `text`, the command prefix that speaks on that channel (`ooc`, `gossip`, `tell {speaker}`), and whether the channel is private. A `ChannelRuleSet` is an ordered list: patterns are anchored to the start of the line, matched against the ANSI-stripped text, the first rule that matches wins, and a speaker has to be one word of letters, so a line is never two messages and prose that reads like a channel is left alone. An unreadable pattern is dropped rather than fatal, and every match runs under a 50 ms timeout.

The shipped defaults live in the embedded resource `Channels/families.json`, keyed by codebase family: `smaug`, `swr`, `diku`, `rom`, `circle`, `lp` and `generic`. A family may open with `{"include": "<family>"}` to inherit the rules it derives from, which is how `swr` builds on `smaug` and how `rom` and `circle` build on `diku`. `ChannelFamilies.Match` maps the directory's free-text codebase onto a family, and `ChannelFamilies.For` picks the world's own rules first, then its family, then generic. `ConnectionProfile` carries both: `Codebase`, merged from the catalog listing on open the same way a world theme is, and `ChannelRules`, which is what a future channel pack will deliver.

`ChannelClassifier` buffers received text into complete server lines through a two-line `AnsiTerminal`, so a network chunk that ends mid line waits and the panel keeps the styled runs the transcript already parsed. A following line that starts with whitespace extends the message above it. When a world speaks GMCP, `Comm.Channel.Text` is routed directly by its own `channel` and `talker` rather than by pattern, and the printed copy of the same line that most worlds send a moment later is dropped on an exact text match within one second.

The controller runs this on public output only, behind the same privacy gate as the agent feed, and raises `ChannelMessageReceived`. The agent's chat filter now asks the same classifier, so the agent and the panel always agree on what chat is.
